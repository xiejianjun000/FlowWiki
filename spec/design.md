...
第二步：指令匹配 L7 场景 skill？
  ├─ "生成迎检清单" → 场景 skill（00_首页/03_实战场景/迎检准备/skills.md）
  ├─ "拆解许可证" → 场景 skill（00_首页/03_实战场景/排污许可证/skills.md）
  ├─ "评审案卷" → 场景 skill（00_首页/03_实战场景/案卷评查/skills.md）
  └─ "溯源 O3 污染" → 场景 skill（00_首页/03_实战场景/大气溯源/skills.md）
  ↓ 不匹配
...
|------|-------------|--------------|
| 作用域 | 所有场景可用 | 仅当前场景可用 |
| 示例 | ingest/query/lint/research | 迎检清单生成/许可证拆解/案卷评审 |
...
请根据目标企业的行业类型和排污许可证内容，生成一份现场迎检清单...
```
...
| 库 | 在 FlowWiki 中的角色 |
|----|------------------|
| 执法督察评查 | 工程化工具链（_scripts/）+ 持续运营看板的参考来源 |
| 环评与排污许可 | Agent bootstrap（CLAUDE.md/AGENTS.md）+ 4 skill 的参考来源 |
| 企业合规AI管家 | L7 场景层 + 行业路由的参考来源 |
| 大气溯源 | 1000 条判据作为 L7 大气溯源场景的素材 |

M5 阶段会从这 4 个库抽取最佳实践填充 FlowWiki 的 L7 参考实现。
