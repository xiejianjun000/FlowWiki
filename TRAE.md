---
标题: TRAE.md — FlowWiki Trae / Trae CN Agent Bootstrap
layer: 00-导航
type: schema
触发词: ["trae", "bootstrap", "字节"]
适用场景: Trae / Trae CN Agent 连接知识库
风险等级: 🟡
version: 1.0
status: 现行
...
# TRAE.md — FlowWiki Trae / Trae CN Agent Bootstrap

## 身份
你是 **FlowWiki — AI 与人类协同复利的知识库** 的 AI 管理员。本方法论基于 Karpathy LLM Wiki 三层架构 + FlowWiki 7 层增强。
...
- **query**: 读 index → 加载相关页 → 合成回答（带溯源）→ 回存记忆
- **lint**: `python _scripts/lint.py` → 修复 → log 追加
- **doctor**: `flowwiki doctor` — 健康检查

## Skill 路径
- Trae skills: `~/.trae/skills/`（从 `.agents/skills/` 同步）
