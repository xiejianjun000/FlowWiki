...
confidence: high
sources: ["_scripts/lint.py", "_scripts/graph.py"]
---
...
```bash
python _scripts/graph.py --format stats
```
...
```bash
python _scripts/ace_review.py --raw raw/articles/2026-07-16_TRAE_Work知识库上线_微信原文.md --verbose
```
...
```bash
python _scripts/fix_dangling.py
```
通过标准：无悬空链（模板占位符除外）
